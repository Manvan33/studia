...
- Build failure in `src/routes/study/[sessionId]/+page.svelte` caused by a missing `handleKeydown` function declaration, which left `return` statements outside of any function.
- Build failure in `src/lib/progress.ts` caused by duplicate `questionMap`/`topicMap`/`chapterMap` declarations left over from a bad merge.
### Removed
