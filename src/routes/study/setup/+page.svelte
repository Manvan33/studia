<script lang="ts">
	...
	async function startSession() {
		...
		try {
			const type: SessionType = wrongOnly ? 'wrong_only' : finalAssessmentOnly ? 'final_assessment' : 'custom';
...
<div class="mx-auto max-w-2xl space-y-6">
	...
	<div class="space-y-4">
		...
		{#if chapters.length > 0}
			<div>
				<p class="mb-2 text-sm font-medium text-gray-700">Chapters</p>
				<div class="space-y-2">
					{#each chapters as chapter}
						<label class="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 cursor-pointer hover:bg-gray-50">
							<input
								type="checkbox"
			...
			<div>
				...
				<div class="space-y-2">
					{#each topics as topic}
						<label class="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2 cursor-pointer hover:bg-gray-50">
							<input
								type="checkbox"
		...
		<div>
			...
			<div class="space-y-2">
				<label class="flex items-center gap-3 cursor-pointer">
					<input type="checkbox" bind:checked={wrongOnly} class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
					<span class="text-sm text-gray-700">Wrong answers only</span>
				</label>
				<label class="flex items-center gap-3 cursor-pointer">
					<input type="checkbox" bind:checked={unansweredOnly} class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
					<span class="text-sm text-gray-700">Unanswered questions only</span>
				</label>
				<label class="flex items-center gap-3 cursor-pointer">
					<input type="checkbox" bind:checked={finalAssessmentOnly} class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
